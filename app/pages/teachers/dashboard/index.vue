...
<template>
  <div class="min-h-screen bg-[#F8FAFC] flex font-sans text-[#2F3E46]">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <main
        class="flex-1 p-6 space-y-6 overflow-y-auto max-w-7xl w-full mx-auto"
      >
        <section
          class="bg-gradient-to-br from-[#FF758F] to-[#FF4D6D] rounded-[2rem] p-6 sm:p-8 md:p-10 text-white relative overflow-hidden shadow-lg shadow-pink-100/50"
        >
          <div
            class="absolute right-0 top-0 w-80 h-80 bg-white/10 rounded-full blur-2xl pointer-events-none -mr-20 -mt-20"
          ></div>
          <div
            class="absolute left-1/3 bottom-0 w-60 h-60 bg-pink-300/20 rounded-full blur-2xl pointer-events-none -mb-20"
          ></div>
          ...
          <div class="relative z-10 space-y-2 max-w-2xl">
            ...
            <p
              class="font-nunito text-xs sm:text-sm text-pink-50/90 font-medium"
            >
              วันนี้มีวิชา{{
                teacherProfile.subject || "เรียน"
              }}ที่ต้องจัดการเช็คชื่อเข้าแถว {{ todayClassesCount }} คาบเรียน
              ยอดรวมนักเรียนเข้าข่ายประเมิน {{ todayStudentsCount }} คน
            </p>
          </div>
        </section>
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <div class="min-w-0">
              <span
                class="text-[10px] text-slate-400 font-bold block uppercase tracking-wide"
                >{{ item.title }}</span
              >
              <span
                class="font-fredoka text-xl font-extrabold text-slate-800 leading-snug block mt-0.5"
                >{{ item.value }}</span
              >
              <span
                class="text-[10px] text-slate-500 font-medium truncate block"
                >{{ item.desc }}</span
              >
            </div>
          </div>
        ...
        <div class="grid grid-cols-12 gap-6">
          <section class="col-span-12 lg:col-span-8 space-y-4">
            <div class="flex items-center justify-between">
              <div>
                <h3
                  class="font-fredoka text-base font-extrabold text-slate-800 leading-none"
                >
                  ห้องเรียนของฉัน
                </h3>
                <span
                  class="text-[10px] text-slate-400 font-bold block mt-1.5 uppercase"
                  >MY RESPONSIBLE CLASSES</span
                >
              </div>
              <button
                @click="
                  showToast('กำลังโหลดข้อมูลห้องเรียนใหม่ล่าสุด', 'success')
                "
              ...
              >
                <div class="space-y-1">
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <span
                      class="text-[10px] text-slate-400 font-semibold flex items-center gap-1"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                  ...
                  <h4
                    class="font-fredoka text-sm font-extrabold text-slate-800 mt-2 truncate"
                  >
                    {{ cls.name }}
                  </h4>
                  <p
                    class="font-nunito text-[11px] text-slate-400 font-semibold truncate"
                  >
                    {{ cls.subject }}
                  </p>
                </div>
                ...
                <div class="mt-4 space-y-2">
                  <div class="flex items-center justify-between text-[11px]">
                    <span class="text-slate-500 font-medium"
                      >ความคืบหน้าการเช็คชื่อ</span
                    >
                    <span class="font-bold text-slate-700"
                      >{{ cls.checkedCount }}/{{ cls.studentsCount }} คน</span
                    >
                  </div>
                  ...
                  >
                    <div
                      ...
                      :style="{
                        width: `${(cls.checkedCount / cls.studentsCount) * 100}%`,
                      }"
                    ></div>
                  </div>

                  <div
                    class="flex items-center justify-between text-[10px] pt-1"
                  >
          ...
          <section class="col-span-12 lg:col-span-4 space-y-4">
            <div>
              <h3
                class="font-fredoka text-base font-extrabold text-slate-800 leading-none"
              >
                นักเรียนที่มาสาย / ลาเรียน
              </h3>
              <span
                class="text-[10px] text-slate-400 font-bold block mt-1.5 uppercase"
                >EXCEPTIONS REPORT TODAY</span
              >
            </div>

            <div
              class="bg-white border border-slate-100 rounded-2xl p-4 shadow-xs space-y-3"
            >
              <div
                v-for="exp in exceptions"
                :key="exp.id"
                class="flex flex-wrap items-center justify-between gap-2 p-2.5 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100/60"
              >
                <div class="flex items-center gap-3 min-w-[120px] flex-1">
                  ...
                  <div class="min-w-0">
                    <p
                      class="text-[11px] font-bold text-slate-700 truncate leading-snug"
                    >
                      {{ exp.name }}
                    </p>
                    <span
                      class="text-[9px] text-slate-400 font-semibold block mt-0.5"
                      >ห้อง {{ exp.room }} • {{ exp.time }}</span
                    >
                  </div>
                </div>
                ...
                <span
                  :class="[
                    'text-[9px] font-bold px-2 py-0.5 rounded-full border max-w-full truncate',
                    exp.badgeClass,
                  ]"
                >
                  {{ exp.statusText }}
              ...
              <div v-if="exceptions.length === 0" class="text-center py-6">
                <span class="text-xs text-slate-400 font-medium"
                  >ไม่มีข้อมูลยกเว้นสำหรับวันนี้</span
                >
              </div>
            </div>

            <div
              class="bg-indigo-50/50 border border-indigo-100/50 rounded-2xl p-4 flex gap-3.5"
            >
              <div
                class="w-8 h-8 rounded-lg bg-indigo-500/10 text-indigo-600 flex items-center justify-center flex-shrink-0"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
              ...
              <div class="min-w-0">
                <h5 class="text-xs font-bold text-slate-700 leading-snug">
                  แจ้งเตือนระบบซิงก์ข้อมูล
                </h5>
                <p
                  class="text-[10px] text-slate-400 font-semibold leading-relaxed mt-1"
                >
                  ระบบทำการอัปเดตข้อมูลนักเรียนและเวลาเรียนเสร็จสมบูรณ์
                  ทุกรายวิชาได้บันทึกเข้าระบบส่วนกลางเรียบร้อยแล้ว
                </p>
              </div>
            </div>
          </section>
