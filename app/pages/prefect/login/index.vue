...
useHead({
  ...
  meta: [
    { name: 'description', content: 'หน้าเข้าสู่ระบบสำหรับสารวัตรนักเรียน เพื่อเข้าเช็คชื่อการเข้าแถวและบันทึกเวลาเรียน' }
  ]
})
...
<template>
  <div class="bg-gradient-to-br from-[#EEF2FF] via-[#F5F3FF] to-[#FAF5FF] min-h-screen relative overflow-hidden flex items-center justify-center p-4 sm:p-6 md:p-8 font-sans">
    ...
    <div class="absolute top-[5%] left-[5%] w-48 h-48 bg-indigo-300/20 rounded-full blur-3xl animate-float-slow pointer-events-none"></div>
    <div class="absolute bottom-[8%] right-[5%] w-72 h-72 bg-purple-300/20 rounded-full blur-3xl pointer-events-none"></div>
    <div class="absolute top-[35%] right-[15%] w-36 h-36 bg-pink-300/15 rounded-full blur-2xl animate-float-medium pointer-events-none"></div>
    <div class="absolute bottom-[5%] left-[10%] w-60 h-60 bg-blue-300/20 rounded-full blur-3xl animate-drift pointer-events-none"></div>
    ...
    <div class="max-w-5xl w-full grid grid-cols-1 md:grid-cols-12 gap-8 items-center bg-white/85 backdrop-blur-2xl border border-white rounded-[2.5rem] p-4 sm:p-6 md:p-8 shadow-2xl relative z-10">
      ...
      <div class="col-span-12 md:col-span-5 hidden md:flex flex-col items-center text-center justify-center p-4">
        <div class="bg-white/80 border border-white/70 rounded-3xl p-6 shadow-md max-w-sm mb-4 transform -rotate-1 hover:rotate-0 transition-transform duration-300">
          <h2 class="font-fredoka text-xl font-bold text-indigo-600 mb-1">ระบบลงเวลาสารวัตรนักเรียน</h2>
          <p class="font-nunito text-slate-500 text-xs sm:text-sm leading-relaxed">
            แผงควบคุมการบันทึกเวลาเข้าเรียนหน้าประตูโรงเรียน สำหรับเจ้าหน้าที่สารวัตรเพื่อเช็คชื่อนักเรียน
          </p>
        </div>
        ...
        <div class="text-indigo-600 font-fredoka text-sm font-semibold tracking-wider flex items-center gap-1.5 mt-2 bg-indigo-50 border border-indigo-100 px-4 py-2 rounded-full shadow-inner animate-pulse">
          <span class="inline-block w-2.5 h-2.5 bg-indigo-500 rounded-full animate-ping"></span>
      ...
      <div 
        :class="['col-span-12 md:col-span-7 bg-white/95 rounded-[2rem] p-6 sm:p-8 md:p-10 shadow-xl border border-indigo-100/30 transition-all duration-300', { 'animate-shake': hasError }]"
      >
        ...
        <div class="flex flex-col items-center text-center md:hidden mb-6">
          ...
          />
          <h2 class="font-fredoka text-2xl font-bold text-indigo-600 mb-1">ระบบสำหรับสารวัตรนักเรียน</h2>
          <p class="font-nunito text-slate-500 text-xs">บันทึกเวลาเรียนหน้าประตูโรงเรียน</p>
        </div>

        <div class="hidden md:block mb-8">
          <span class="bg-indigo-50 text-indigo-600 text-xs font-bold font-fredoka tracking-wider px-3.5 py-1.5 rounded-full uppercase border border-indigo-100/50">
            PREFECT PORTAL
          </span>
          <h1 class="font-fredoka text-3xl font-extrabold text-slate-800 mt-4 mb-2">
            ลงชื่อเข้าใช้งาน
          </h1>
        ...
        <form @submit.prevent="handleLogin" class="space-y-5">
          <!-- Username/Inspector ID Field -->
          <div class="space-y-1.5">
            <label for="inspectorId" class="block font-fredoka text-sm font-bold text-slate-700 flex items-center gap-1.5 pl-1">
          ...
          <div class="space-y-1.5">
            <div class="flex justify-between items-center pl-1">
              <label for="password" class="block font-fredoka text-sm font-bold text-slate-700 flex items-center gap-1.5">
                รหัสผ่าน
              </label>
              <span 
            ...
            <label class="flex items-center gap-2 cursor-pointer group text-slate-600 select-none">
              <div class="relative">
                <input 
                  ...
                  :disabled="isLoading || isSuccess"
                />
                <div class="w-5 h-5 bg-slate-100 border-2 border-slate-200 rounded-md transition-all duration-200 peer-checked:bg-indigo-600 peer-checked:border-indigo-600 flex items-center justify-center">
                  <!-- Checkmark icon -->
        ...
        <!-- Back to teacher portal link -->
        <!-- <div class="mt-8 text-center border-t border-slate-100 pt-6">
          <p class="text-xs font-semibold text-slate-400">
            คุณคือคุณครูใช่หรือไม่?
            <NuxtLink 
              to="/teachers/login" 
              class="text-indigo-600 font-extrabold hover:text-indigo-700 underline hover:underline transition-colors ml-1.5 inline-block group"
            >
              เข้าสู่ระบบของครูที่นี่
