...
export const useSettingForm = (showToast: (msg: string, type?: 'success' | 'error' | 'warning' | 'info') => void) => {
  ...
  const loadSettings = async () => {
    ...
    try {
      const res: any = await $fetch(`http://localhost:8080/api/v1/auth/settings/${session.value.teacher_id}`)
      if (res.data) {
        ...
        formProfile.value.title = res.data.subject_taught ? `คุณครูประจำวิชา${res.data.subject_taught}` : ''
  ...
  const saveSettings = async () => {
    ...
    isSaving.value = true
    try {
      const subjectClean = formProfile.value.title.replace('คุณครูประจำวิชา', '').trim()
      await $fetch(`http://localhost:8080/api/v1/auth/settings/${session.value.teacher_id}`, {
        method: 'PATCH',
        body: {
      ...
      const errorMsg = err.data?.message || 'เกิดข้อผิดพลาดในการบันทึกการตั้งค่า'
  ...
  const changePassword = async () => {
    ...
    isSaving.value = true
    try {
      await $fetch(`http://localhost:8080/api/v1/auth/settings/${session.value.teacher_id}/password`, {
        method: 'PATCH',
        body: {
