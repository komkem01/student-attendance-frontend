...
export const useLoginForm = (
  ...
  const handleLogin = async () => {
    ...
    try {
      // 🟢 อัปเดต: เปลี่ยนมาใช้ useCustomFetch สำหรับการ Login
      const res: any = await useCustomFetch("/auth/login", {
        method: "POST",
        body: {
      ...
      const errorMsg =
        err.data?.message ||
        "อีเมลหรือรหัสผ่านไม่ถูกต้อง กรุณาตรวจสอบข้อมูลและลองใหม่อีกครั้ง";
