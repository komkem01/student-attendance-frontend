...
export const usePrefectCheckIn = (showToast: (msg: string, type?: 'success' | 'error' | 'warning') => void) => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [studentsRes, classroomsRes, studentClassroomsRes, schedulesRes, attendancesRes, prefixesRes]: any = await Promise.all([
        ...
        $fetch('http://localhost:8080/api/v1/system/prefix')
      ])
  ...
  const getClassroomName = (studentId: string) => {
    const mapping = studentClassroomsList.value.find(sc => sc.student_id === studentId)
  ...
  const checkedInIds = computed(() => {
    ...
    const todayAttendances = attendancesList.value.filter(a => a.date === todayStr && (a.status === 'present' || a.status === 'late'))
  ...
  const checkInHistory = computed(() => {
    ...
    const todayAttendances = attendancesList.value.filter(a => a.date === todayStr && a.status === 'late')
    
    // Map to the format needed by UI list
    const records = todayAttendances.map(a => {
      const student = allStudents.value.find(s => s.id === a.student_id)
      if (!student) return null

      const timeVal = a.updated_at ? new Date(a.updated_at) : new Date()
      const timeStr = timeVal.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })
  ...
  const presentCount = computed(() => {
    const todayStr = new Date().toISOString().split('T')[0]
    return attendancesList.value.filter(a => a.date === todayStr && a.status === 'present').length
  })

  const lateCount = computed(() => {
    const todayStr = new Date().toISOString().split('T')[0]
    return attendancesList.value.filter(a => a.date === todayStr && a.status === 'late').length
  })

  const totalCheckedCount = computed(() => {
    const todayStr = new Date().toISOString().split('T')[0]
    return attendancesList.value.filter(a => a.date === todayStr && (a.status === 'present' || a.status === 'late')).length
  })
  ...
  const confirmCheckIn = async (status: 'present' | 'late') => {
    ...
    // 1. Find classroom ID
    const mapping = studentClassroomsList.value.find(sc => sc.student_id === s.id)
    if (!mapping) {
    ...
    const todayStr = new Date().toISOString().split('T')[0]
    const schedule = schedulesList.value.find(sc => sc.classroom_id === mapping.classroom_id && sc.date === todayStr)
    if (!schedule) {
      showToast('ห้องเรียนนี้ไม่มีตารางเรียนในวันนี้ จึงไม่สามารถเช็คชื่อได้', 'warning')
      isSubmitting.value = false
      return
    }
    ...
    try {
      ...
      const res: any = await $fetch('http://localhost:8080/api/v1/classroom-attendences', {
        method: 'POST',
        body: {
          student_id: s.id,
      ...
      if (res.data) {
        // Reload attendances
        const attendancesRes: any = await $fetch('http://localhost:8080/api/v1/classroom-attendences')
        attendancesList.value = attendancesRes.data || []
  ...
  const applyEditStatus = async (newStatus: 'present' | 'late') => {
    ...
    try {
      ...
      let lateMinutes = 0
      if (newStatus === 'late') {
        const schedule = schedulesList.value.find(s => s.id === dbRecord.schedule_id)
      ...
      const res: any = await $fetch(`http://localhost:8080/api/v1/classroom-attendences/${rec.id}`, {
        method: 'PATCH',
        body: {
  ...
  const deleteRecord = async () => {
    ...
    try {
      await $fetch(`http://localhost:8080/api/v1/classroom-attendences/${rec.id}`, {
        method: 'DELETE'
      })
