...
export const useStudentList = () => {
  ...
  const fetchStudentsData = async () => {
    ...
    try {
      ...
      const [classroomsRes, schedulesRes, studentClassroomsRes, studentsRes, attendancesRes, prefectsRes]: any = await Promise.all([
      ...
      const formattedClassrooms = teacherClassrooms.map((cls: any) => {
        ...
        if (activeSchedule) {
          ...
          const scheduleAttendance = rawAttendances.filter((a: any) => a.schedule_id === activeSchedule.id)
          if (scheduleAttendance.length > 0) {
            statusVal = 'completed'
            // Find most recent attendance edit time
            const lastUpdated = scheduleAttendance.reduce((latest: string, curr: any) => {
              return new Date(curr.updated_at).getTime() > new Date(latest).getTime() ? curr.updated_at : latest
            }, scheduleAttendance[0].updated_at)
            
            const timeStr = new Date(lastUpdated).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })
  ...
  const handleAddStudent = async () => {
    ...
    try {
      ...
      if (targetClass) {
        ...
        const noTaken = targetClass.students.some((s: any) => s.no === Number(newStudentNo.value))
        if (noTaken) {
      ...
      console.error(err)
      const msg = err.data?.message || 'เกิดข้อผิดพลาดในการบันทึกข้อมูลนักเรียน'
      showToast(msg, 'error')
    } finally {
      isSubmitting.value = false
    }
  ...
  const handleUpdateStudent = async () => {
    ...
    try {
      ...
      if (targetClass) {
        ...
        if (editingStudentObj.value.classId !== editStudentClassId.value || editingStudentObj.value.no !== Number(editStudentNo.value)) {
          const noTaken = targetClass.students.some((s: any) => s.id !== editingStudentObj.value.id && s.no === Number(editStudentNo.value))
          if (noTaken) {
      ...
      await $fetch(`http://localhost:8080/api/v1/students/${editingStudentObj.value.id}`, {
        method: 'PATCH',
        body: {
          gender_id: genderId,
          prefix_id: prefixId,
      ...
      if (editingStudentObj.value.classId !== editStudentClassId.value) {
        const mappingRecord = studentClassroomsList.value.find(
          sc => sc.student_id === editingStudentObj.value.id && sc.classroom_id === editingStudentObj.value.classId
        )

        if (mappingRecord) {
          await $fetch(`http://localhost:8080/api/v1/student-classrooms/${mappingRecord.id}`, {
            method: 'PATCH',
            body: {
      ...
      console.error(err)
      const msg = err.data?.message || 'เกิดข้อผิดพลาดในการอัปเดตข้อมูลนักเรียน'
      showToast(msg, 'error')
    } finally {
      isSubmitting.value = false
    }
  ...
  const confirmDeleteStudent = (student: any) => {
    ...
    const isPrefect = prefectsList.value.some((p: any) => p.student_id === student.id)
    if (isPrefect) {
      showToast('ไม่สามารถลบนักเรียนคนนี้ได้ เนื่องจากมีสถานะเป็นสารวัตรนักเรียน', 'error')
      return
    }
  ...
  const handleDeleteStudent = async () => {
    ...
    try {
      ...
      const mappingRecord = studentClassroomsList.value.find(
        sc => sc.student_id === deletingStudentObj.value.id && sc.classroom_id === deletingStudentObj.value.classId
      )
      if (mappingRecord) {
        await $fetch(`http://localhost:8080/api/v1/student-classrooms/${mappingRecord.id}`, {
          method: 'DELETE'
      ...
      await $fetch(`http://localhost:8080/api/v1/students/${deletingStudentObj.value.id}`, {
        method: 'DELETE'
      })
