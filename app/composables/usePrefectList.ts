...
export const usePrefectList = () => {
  ...
  const fetchPrefectsData = async () => {
    ...
    try {
      const [classroomsRes, studentClassroomsRes, studentsRes, prefectsRes, attendancesRes, schedulesRes]: any = await Promise.all([
      ...
      classroomsList.value = teacherClassrooms.map((cls: any) => {
        ...
        const studentsCount = rawStudentClassrooms.filter((sc: any) => sc.classroom_id === cls.id).length
        return {
          id: cls.id,
          name: cls.class,
          grade,
      ...
      const teacherPrefects = rawPrefects.filter((p: any) => teacherClassroomIds.includes(p.classroom_id))

      prefects.value = teacherPrefects.map((pref: any) => {
        const studentInfo = rawStudents.find((s: any) => s.id === pref.student_id)
        const firstName = studentInfo ? studentInfo.firstname : 'ไม่พบข้อมูล'
        const lastName = studentInfo ? studentInfo.lastname : ''

        const classroomInfo = rawClassrooms.find((c: any) => c.id === pref.classroom_id)
        const className = classroomInfo ? classroomInfo.class : ''
        const { grade, room } = parseClassroomDetails(className)

        // Calculate checks today in this classroom
        const todayStr = new Date().toISOString().split('T')[0]
        const classroomAttendances = rawAttendances.filter((a: any) => {
          const isToday = a.date.startsWith(todayStr)
          const schedule = rawSchedules.find((s: any) => s.id === a.schedule_id)
          const isForClassroom = schedule && schedule.classroom_id === pref.classroom_id
          return isToday && isForClassroom
        })
        ...
        return {
          ...
          lastLogin: pref.last_login ? new Date(pref.last_login).toLocaleString('th-TH') : 'ยังไม่เคยเข้าสู่ระบบ',
          checksToday: classroomAttendances.length,
  ...
  const filteredStudentsForClass = computed(() => {
    ...
    return mappings.map((sc: any) => {
      const s = students.value.find((student: any) => student.id === sc.student_id)
      if (!s) return null
      return {
        id: s.id,
  ...
  const selectedClassroomLabel = computed(() => {
    const cls = classroomsList.value.find(c => c.id === selectedClassroomId.value)
    if (cls) return cls.name
    if (selectedPrefect.value && selectedClassroomId.value === selectedPrefect.value.classroomId) {
      const g = selectedPrefect.value.grade
      const r = selectedPrefect.value.room
      if (g.startsWith('ม.')) {
        const gradeNum = g.substring(2)
        return r ? `ชั้นมัธยมศึกษาปีที่ ${gradeNum}/${r}` : `ชั้นมัธยมศึกษาปีที่ ${gradeNum}`
      }
  ...
  const selectedStudentLabel = computed(() => {
    if (selectedPrefect.value && selectedStudentId.value === selectedPrefect.value.studentId) {
      return `${selectedPrefect.value.firstName} ${selectedPrefect.value.lastName}`
    }
    const stu = filteredStudentsForClass.value.find(s => s.id === selectedStudentId.value)
  ...
  const totalActive = computed(() => prefects.value.filter(p => p.isActive).length)
  const totalToday = computed(() => prefects.value.reduce((sum, p) => sum + p.checksToday, 0))
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      showToast(`เพิ่มสารวัตร ${form.value.firstName} ${form.value.lastName} เรียบร้อยแล้ว`, 'success')
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await $fetch(`http://localhost:8080/api/v1/prefects/${selectedPrefect.value.id}`, {
        method: 'PATCH',
        body: payload
      })
