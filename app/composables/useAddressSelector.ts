...
export const useAddressSelector = () => {
  ...
  const fetchDistricts = async (provinceID: string) => {
    try {
      const res: any = await $fetch(`http://localhost:8080/api/v1/system/district?province_id=${provinceID}`)
  ...
  const fetchSubdistricts = async (districtID: string) => {
    try {
      const res: any = await $fetch(`http://localhost:8080/api/v1/system/sub-district?district_id=${districtID}`)
  ...
  const fetchZipcode = async (subdistrictID: string) => {
    try {
      const res: any = await $fetch(`http://localhost:8080/api/v1/system/zipcode?sub_district_id=${subdistrictID}`)
