...
export const useTeacherDashboard = () => {
  ...
  const loadLookups = async () => {
    ...
    try {
      const res: any = await $fetch('http://localhost:8080/api/v1/system/prefix')
      ...
      const formattedClassrooms = teacherClassrooms.map((cls: any) => {
        ...
        if (activeSchedule) {
          ...
          if (scheduleAttendances.length > 0 && enrolledStudents.length > 0) {
            ...
            const presentOrLate = scheduleAttendances.filter((a: any) => a.status === 'present' || a.status === 'late').length
            attendanceRate = Math.round((presentOrLate / enrolledStudents.length) * 100)
        ...
        return {
          ...
          checkedCount: status === 'completed' ? enrolledStudents.length : enrolledStudents.filter((s: any) => s.status !== '').length,
          status,
          attendanceRate,
          time: timeText,
      ...
      formattedClassrooms.forEach((room: any) => {
        ...
        if (room.rawDate === todayStr) {
          room.students.forEach((std: any) => {
            if (std.status === 'absent' || std.status === 'leave' || std.status === 'late') {
  ...
  const stats = computed(() => {
    ...
    const todayRooms = classrooms.value.filter((r: any) => r.rawDate === todayStr)
    ...
    const avgAttendanceRateVal = totalCheckedToday > 0 
      ? Math.round(((totalPresent + totalLate) / totalCheckedToday) * 1000) / 10
      : 100.0 // default to 100% if no students checked yet

    // 2. Absentees description
    const absenteesByRoom: string[] = []
    todayRooms.forEach((room: any) => {
      const roomAbsents = room.students.filter((s: any) => s.status === 'absent').length
      if (roomAbsents > 0) {
        const roomNameShort = room.name.replace('ชั้นมัธยมศึกษาปีที่ ', 'ม.')
        absenteesByRoom.push(`${roomNameShort} (${roomAbsents} คน)`)
      }
    })
    const absenteesDesc = absenteesByRoom.length > 0 
    ...
    return [
      {
        ...
        desc: todayRooms.length > 0 ? 'คำนวณจากคาบเรียนวันนี้' : 'ไม่มีคาบเรียนวันนี้',
