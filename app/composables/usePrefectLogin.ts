...
export const usePrefectLogin = (
  ...
  const handleLogin = async () => {
    ...
    try {
      // 🟢 อัปเดต: เปลี่ยนมาใช้ useCustomFetch สำหรับการ Login ของสารวัตรนักเรียน
      const res: any = await useCustomFetch("/auth/prefect-login", {
        method: "POST",
        body: {
      ...
      showToast(
        "เข้าสู่ระบบสำเร็จ กำลังเข้าสู่หน้าบันทึกเวลาเรียน...",
        "success",
      );
      setTimeout(() => {
      ...
      const errorMsg =
        err.data?.message ||
        "รหัสประจำตัวหรือรหัสผ่านไม่ถูกต้อง กรุณาตรวจสอบและลองใหม่อีกครั้ง";
